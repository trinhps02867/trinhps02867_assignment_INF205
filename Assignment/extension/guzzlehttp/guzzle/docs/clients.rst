...
base_url
    ...
    .. code-block:: php
        ...
        $client = new app\extension\guzzlehttp\guzzle\src\Client(['base_url' => 'https://github.com']);
...
adapter
    Configures the HTTP adapter (``app\extension\guzzlehttp\guzzle\src\Adapter\AdapterInterface``) used
...
parallel_adapter
    ...
    (``app\extension\guzzlehttp\guzzle\src\Adapter\ParallelAdapterInterface``). Guzzle will by default
...
message_factory
    ...
    (``app\extension\guzzlehttp\guzzle\src\Message\MessageFactoryInterface``).
...
emitter
    Specifies an event emitter (``app\extension\guzzlehttp\guzzle\src\Event\EmitterInterface``) instance
...
- ``app\extension\guzzlehttp\guzzle\src\Client::get``: Sends a GET request.
- ``app\extension\guzzlehttp\guzzle\src\Client::head``: Sends a HEAD request
- ``app\extension\guzzlehttp\guzzle\src\Client::post``: Sends a POST request
- ``app\extension\guzzlehttp\guzzle\src\Client::put``: Sends a PUT request
- ``app\extension\guzzlehttp\guzzle\src\Client::delete``: Sends a DELETE request
- ``app\extension\guzzlehttp\guzzle\src\Client::options``: Sends an OPTIONS request
...
a client, a ``app\extension\guzzlehttp\guzzle\src\Exception\RequestException`` is thrown.
...
``app\extension\guzzlehttp\guzzle\src\Exception\RequestException`` always contains a
``app\extension\guzzlehttp\guzzle\src\Message\RequestInterface`` object that can be accessed using the
...
``app\extension\guzzlehttp\guzzle\src\Message\ResponseInterface`` using the ``getResponse()`` method of
the exception.
...
``app\extension\guzzlehttp\guzzle\src\Exception\ClientErrorResponseException`` for 4xx level HTTP
responses and ``app\extension\guzzlehttp\guzzle\src\Exception\ServerException`` for 5xx level responses,
both of which extend from ``app\extension\guzzlehttp\guzzle\src\Exception\BadResponseException``.
...
``app\extension\guzzlehttp\guzzle\src\Message\RequestInterface`` objects. In addition to providing the
...
The ``app\extension\guzzlehttp\guzzle\src\Event\ErrorEvent`` event object is emitted when an error
...
different ``app\extension\guzzlehttp\guzzle\src\Message\ResponseInterface`` object. See :doc:`events`
...
function ``app\extension\guzzlehttp\guzzle\src\batch()`` that makes this very simple:
...
.. code-block:: php
    ...
    $results = app\extension\guzzlehttp\guzzle\src\batch($client, $requests);
...
``app\extension\guzzlehttp\guzzle\src\batch()`` accepts an optional associative array of options in the
...
the options array exposed in ``app\extension\guzzlehttp\guzzle\src\ClientInterface::sendAll()``.
...
.. code-block:: php
    ...
    $client = new app\extension\guzzlehttp\guzzle\src\Client(['base_url' => 'http://httpbin.org']);
...
:Types:
    ...
    - ``app\extension\guzzlehttp\guzzle\src\Post\PostBodyInterface``
:Default: None
...
- Array
  ...
  ``app\extension\guzzlehttp\guzzle\src\Post\PostBodyInterface`` object as the body.
...
- ``app\extension\app\extension\guzzlehttp\guzzle\src\streams\src\StreamInterface``
  ...
  .. code-block:: php
      ...
      $stream = app\extension\app\extension\guzzlehttp\guzzle\src\streams\src\Stream::factory('contents...');
...
:Types:
    - array
    - ``app\extension\guzzlehttp\guzzle\src\Query``
:Default: None
...
.. code-block:: php
     ...
     */
    class FooAuth implements app\extension\guzzlehttp\guzzle\src\Event\SubscriberInterface
    {
        private $password;
...
:Types:
    - bool
    - array
    - ``app\extension\guzzlehttp\guzzle\src\Cookie\CookieJarInterface``
:Default: None
...
Set to a ``app\extension\guzzlehttp\guzzle\src\Cookie\CookieJarInterface`` object to use an existing
cookie jar.

.. code-block:: php

    $jar = new app\extension\guzzlehttp\guzzle\src\Cookie\CookieJar();
...
Pass a ``app\extension\app\extension\guzzlehttp\guzzle\src\streams\src\StreamInterface`` object to stream the response body
to an open Guzzle stream:

.. code-block:: php

    $resource = fopen('/path/to/file', 'w');
    $stream = app\extension\app\extension\guzzlehttp\guzzle\src\streams\src\Stream::factory($resource);
...
:Summary: Array of event subscribers to add to the request. Each value in the
    array must be an instance of ``app\extension\guzzlehttp\guzzle\src\Event\SubscriberInterface``.
:Types: array
:Default: None
...
.. code-block:: php
    ...
    // Throws a app\extension\guzzlehttp\guzzle\src\Exception\ServerException
...
.. code-block:: php
    ...
    // PHP Fatal error:  Uncaught exception 'app\extension\guzzlehttp\guzzle\src\Exception\RequestException'
...
``app\extension\guzzlehttp\guzzle\src\Common\EventEmitter`` object that can be used to add event
...
``GUZZLE_CURL_SELECT_TIMEOUT``
    Controls the duration in seconds that ``app\extension\guzzlehttp\guzzle\src\Adapter\Curl\MultiAdapter``
