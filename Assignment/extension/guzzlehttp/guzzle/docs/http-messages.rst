...
.. code-block:: php
    ...
    $response = app\extension\guzzlehttp\guzzle\src\get('http://httpbin.org/get');
    if ($response->getBody()) {
...
``app\extension\app\extension\guzzlehttp\guzzle\src\streams\src\StreamInterface``. This stream is used for both uploading
...
``app\extension\app\extension\guzzlehttp\guzzle\src\streams\src\Stream::factory()`` method. This method accepts various
inputs like strings, resources returned from ``fopen()``, and other
``app\extension\app\extension\guzzlehttp\guzzle\src\streams\src\StreamInterface`` objects.
...
a ``app\extension\guzzlehttp\guzzle\src\Message\MessageFactoryInterface`` to create request messages.
...
This method returns a ``app\extension\guzzlehttp\guzzle\src\Query`` object. A Query object can be
...
You can use the ``app\extension\guzzlehttp\guzzle\src\Query::walkQuery()`` static function to easily
...
The config object is a ``app\extension\guzzlehttp\guzzle\src\Common\Collection`` object that acts like
...
Request objects implement ``app\extension\guzzlehttp\guzzle\src\Common\HasEmitterInterface``, so they
...
.. code-block:: php
    ...
    $response = app\extension\guzzlehttp\guzzle\src\get('http://httpbin.org/redirect-to?url=http://www.google.com');
    echo $response->getEffectiveUrl();
    // http://www.google.com
