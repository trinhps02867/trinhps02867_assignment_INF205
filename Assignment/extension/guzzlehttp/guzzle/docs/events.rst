...
``app\extension\guzzlehttp\guzzle\src\Common\HasEmitterInterface`` interface have a
``app\extension\guzzlehttp\guzzle\src\Common\EventEmitter`` object. You can add event *listeners* and
event *subscribers* to an event *emitter*.

emitter
    An object that implements ``app\extension\guzzlehttp\guzzle\src\Common\EventEmitterInterface``. This
...
You can get the event emitter of ``app\extension\guzzlehttp\guzzle\src\Common\HasEmitterInterface``
...
``app\extension\guzzlehttp\guzzle\src\Common\EventInterface`` interface, the name of the event, and the
...
of a ``app\extension\guzzlehttp\guzzle\src\Common\EventInterface`` object:
...
``app\extension\guzzlehttp\guzzle\src\Common\EventSubscriberInterface`` object. They are used to register
...
.. code-block:: php
    ...
    use app\extension\guzzlehttp\guzzle\src\Event\EmitterInterface;
...
``app\extension\guzzlehttp\guzzle\src\Event\RequestEvents`` object.
...
``app\extension\guzzlehttp\guzzle\src\Event\RequestEvents::EARLY``
    Used when you want a listener to be triggered as early as possible in the
    event chain.

``app\extension\guzzlehttp\guzzle\src\Event\RequestEvents::LATE``
    Used when you want a listener to be to be triggered as late as possible in
    the event chain.

``app\extension\guzzlehttp\guzzle\src\Event\RequestEvents::PREPARE_REQUEST``
    ...
    ``app\extension\guzzlehttp\guzzle\src\Subscriber\Prepare`` event subscriber which is responsible for
...
``app\extension\guzzlehttp\guzzle\src\Event\RequestEvents::SIGN_REQUEST``
...
``app\extension\guzzlehttp\guzzle\src\Event\RequestEvents::VERIFY_RESPONSE``
    Used when you want a listener to be triggered when a response is being
    validated during the ``complete`` event. The
    ``app\extension\guzzlehttp\guzzle\src\Subscriber\HttpError`` event subscriber uses this event
...
``app\extension\guzzlehttp\guzzle\src\Event\RequestEvents::REDIRECT_RESPONSE``
    Used when you want a listener to be triggered when a response is being
    redirected during the ``complete`` event. The
    ``app\extension\guzzlehttp\guzzle\src\Subscriber\Redirect`` event subscriber uses this event
...
a ``app\extension\guzzlehttp\guzzle\src\Event\BeforeEvent``.

.. code-block:: php

    use app\extension\guzzlehttp\guzzle\src\Client;
    use app\extension\guzzlehttp\guzzle\src\Common\EmitterInterface;
    use app\extension\guzzlehttp\guzzle\src\Event\BeforeEvent;
    ...
    $request->getEmitter()->on(
        ...
        function (BeforeEvent $e, $name, EmitterInterface $emitter) {
            ...
            // "app\extension\guzzlehttp\guzzle\src\Client"
        }
    );
...
``app\extension\guzzlehttp\guzzle\src\Message\ResponseInterface``. Intercepting the event will prevent
...
emitted is a ``app\extension\guzzlehttp\guzzle\src\Event\HeadersEvent``.
...
response has been received. The event is a ``app\extension\guzzlehttp\guzzle\src\Event\CompleteEvent``.
...
.. attention::
    ...
    Any ``app\extension\guzzlehttp\guzzle\src\Exception\RequestException`` encountered while executing
...
``app\extension\guzzlehttp\guzzle\src\Event\ErrorEvent``.
