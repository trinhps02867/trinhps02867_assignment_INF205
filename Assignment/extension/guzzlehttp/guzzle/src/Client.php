...
use app\extension\guzzlehttp\guzzle\src\Adapter\AdapterInterface;
use app\extension\guzzlehttp\guzzle\src\Adapter\Curl\CurlAdapter;
use app\extension\guzzlehttp\guzzle\src\Adapter\Curl\MultiAdapter;
use app\extension\guzzlehttp\guzzle\src\Adapter\FakeParallelAdapter;
use app\extension\guzzlehttp\guzzle\src\Adapter\ParallelAdapterInterface;
use app\extension\guzzlehttp\guzzle\src\Adapter\StreamAdapter;
...
use app\extension\guzzlehttp\guzzle\src\Adapter\TransactionIterator;
...
use app\extension\guzzlehttp\guzzle\src\Message\MessageFactory;
...
class Client implements ClientInterface
{
    ...
    {
        return \app\extension\guzzlehttp\guzzle\src\deprecation_proxy(
            $this,
            $name,
            $arguments,
    ...
    public function getDefaultOption($keyOrPath = null)
    {
        return $keyOrPath === null
            ? $this->defaults
            : \app\extension\guzzlehttp\guzzle\src\get_path($this->defaults, $keyOrPath);
    }

    public function setDefaultOption($keyOrPath, $value)
    {
        \app\extension\guzzlehttp\guzzle\src\set_path($this->defaults, $keyOrPath, $value);
    }
    ...
    private function buildUrl($url)
    {
        if (!is_array($url)) {
            ...
            return \app\extension\guzzlehttp\guzzle\src\uri_template($url[0], $url[1]);
        }

        return (string) $this->baseUrl->combine(
            \app\extension\guzzlehttp\guzzle\src\uri_template($url[0], $url[1])
        );
    }
    ...
    private function configureBaseUrl(&$config)
    {
        if (!isset($config['base_url'])) {
            ...
            $this->baseUrl = Url::fromString(
                \app\extension\guzzlehttp\guzzle\src\uri_template(
                    $config['base_url'][0],
                    $config['base_url'][1]
                )
            );
